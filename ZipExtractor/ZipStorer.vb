...
#Region "Public methods"
  ...
  Public Function ExtractFile(ByVal _zfe As ZipFileEntry, ByVal _filename As String) As Boolean
    ...
    Dim DirPath As String = System.IO.Path.GetDirectoryName(_filename)

    If DirPath <> "" And Directory.Exists(DirPath) = False Then
      Directory.CreateDirectory(DirPath)
    End If

    ' Check it is directory. If so, do nothing
    If Directory.Exists(_filename) = True Then
      Return True
    End If
  ...
  Public Function ExtractFile(ByVal _zfe As ZipFileEntry, ByVal _stream As Stream) As Boolean
    ...
    If BitConverter.ToUInt32(signature, 0) <> &H4034B50UI Then
      Return False
    End If
  ...
  End Function
      ...
      '/ <returns>True if success, false if not.</returns>
      '/ <remarks>Unique compression methods are Store and Deflate</remarks>
      Public Function ExtractFile(ByVal _zfe As ZipFileEntry, ByRef _file() As Byte) As Boolean
...
'       central file header signature   4 bytes  (&H02014b50)
...
'      Private Function ConvertDosDateTime(ByVal dosDate As Int16, ByVal dosTime As Int16) As DateTime
        'Extract the time
'         Dim intSec As Integer = (dosTime And 31) * 2
'         dosTime = dosTime / (2 ^ 5)                                     'Shift 5

'   bits to the right
'         Dim intMin As Integer = (dosTime And 63)
'         dosTime = dosTime / (2 ^ 6)                                     'Shift 6

'   bits to the right
'         Dim intHrs As Integer = (dosTime And 31)
'         'Extract the date
'         Dim intDay As Integer = (dosDate And 31)
'         dosDate = dosDate / (2 ^ 5)                                     'Shift 5

'   bits to the right
'         Dim intMon As Integer = (dosDate And 15)
'         dosDate = dosDate / (2 ^ 4)                                     'Shift 4

'   bits to the right
'         Dim intYear As Integer = (dosDate And 127) + 1980
'         Return New DateTime(intYear, intMon, intDay, intHrs, intMin, intSec)
'      End Function
